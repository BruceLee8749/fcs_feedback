    ...
    def send_request(self, i, file, data, check_info):
        try:
            ...
            if dict_in(check_info, r_body):  # 判断后者的集合是否包含前者集合的所有元素
                print('测试结果为：通过')
                self.sheet.cell(i, 9, '通过').fill = self.fill_green  # 单元格填充绿色
                ...
                self.sheet.cell(i, 9, '失败').fill = self.fill_red  # 失败填充红色
                try:
                    if 'composite' in self.url:
                        convert_url = r_body['data']['viewUrl']  # 获取转换好的url
            ...
            self.sheet.cell(i, 9, '阻塞').fill = self.fill_yellow  # 阻塞填充黄色
            self.write_execl(i, ex)  # 出现阻塞的话，写入测试结果和异常原因

    def write_execl(self, i, r_body, r_urls=None):
        self.sheet.cell(i, 8, r_body)  # 写入responseBody
        if r_urls:  # 写入返回的url
            if 'composite' in self.url:
                self.sheet.cell(i, 10, r_urls).hyperlink = r_urls  # 填入超链接
            else:
                url_cell = 10  # 写URL的单元格位于第10列
                for r_url in r_urls:  # 多个url的情况下依次写入，这里是针对于dcs接口图片转换的的url是多个
                    if url_cell < 11:
                        ...
                        self.sheet.cell(i, url_cell, '超链接%s' % int(url_cell - 9)).hyperlink = r_url  # 超过一个链接时，通过数字命名
                    url_cell += 1
    ...
    def run(self):
        ...
        for i in range(2, rows + 1):  # 遍历该sheet下的测试用例
            try:
                ...
                if case_name and case_name != '' and self.sheet.cell(i, 9).value != '通过':  # 没有用例名字不会运行
                    ...
                    convert_type = int(self.sheet.cell(i, 5).value)  # 获取convertType
                    other_data = eval(self.sheet.cell(i, 6).value)  # 获取其他参数
                    other_data.update({"convertType": convert_type})  # 把单独列出的convertType填入data中
                    msg = self.sheet.cell(i, 7).value  # 获取验证信息
                    self.send_request(i, file_path, other_data, msg)  # 发送请求
            except Exception as e:
                print('格式错误，' + traceback.format_exc())  # 如果出现测试用例表的格式错误就会出现在这里
                self.sheet.cell(i, 9, '格式错误').fill = self.fill_grey  # 单元格填充灰色
...
class BrowserAction(CellColor):
    ...
    def element_input(self, loc, value, clear=1):  # 改变输入方法
        ...
        try:
            ...
            if clear == 2:  # 键盘清空再输入，clear设置为1
                self.find_ele(loc).send_keys(Keys.CONTROL + 'a')
                self.find_ele(loc).send_keys(Keys.BACKSPACE)
            sleep(2)
    ...
    def select_page(self, file_style, page_num):
        """
        页码输入栏中输入要转到的页码
        file_style:文档类型
        file_style=1,文档类型为doc、docx、ppt、pptx,pdf14
        file_style=2，文档类型为xls、xlsx
        file_style=3,文档类型为ofd
        file_style=4,文档类型为pdf
        : page_num: 跳转页码或sheet表序号
        """
        try:
            if file_style == 1:
                self.element_input("//input[@id='currentPage']", value=page_num, clear=2)  # 当前页输入框中输入要转到的页码
    ...
    def screenshot_save(self, row_num, sheet_name, file_path=fcs_result_path):
        ...
        if assert_pic_exist_or_not(ex_image_path):
            ...
            sheet.cell(row_num, 13, str(case_id) + '.png').hyperlink = ac_image_path  # 以超链接格式写入结果表对应的单元格中
            result = image_contrast(ex_image_path, ac_image_path)
            print(f'-------------正在运行用例:{case_id}的截图对比值为{result}-------------')
            if result > 0:  # 获取对比值大于0，则测试不通过
                sheet.cell(row_num, 14, result)  # 将对比结果写入
                sheet.cell(row_num, 14).fill = self.fill_red  # 错误结果标记红色
            else:
                sheet.cell(row_num, 14, 'pass')  # 对比值等于0，则标记pass
                sheet.cell(row_num, 14).fill = self.fill_green  # 正确结果结果标记绿色
            workbook.save(file_path)  # 保存结果表
            return result
        else:
            self.capture_image_function(ex_image_path)  # 如果不存在，将截图放入期望截图文件夹中
            sheet.cell(row_num, 12, str(case_id) + '.png').hyperlink = ex_image_path  # 以超链接格式写入结果表对应的单元格中
            workbook.save(file_path)  # 保存结果表
            return "不进行对比"
